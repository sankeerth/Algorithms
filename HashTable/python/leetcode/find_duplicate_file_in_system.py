...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        contentToDirectoryPaths = {}
        res = []

        for path in paths:
            dirAndPath = path.split(' ')
            rootDir = dirAndPath[0]

            for i in range(1, len(dirAndPath)):
                name, content = dirAndPath[i].split('(')
                content = content.split(')')[0]
                if content not in contentToDirectoryPaths:
                    contentToDirectoryPaths[content] = []
                contentToDirectoryPaths[content].append(rootDir + '/' + name)

        for content, path in contentToDirectoryPaths.items():
            if len(path) > 1:
                res.append(path)
...
If the file content is very large (GB level), how will you modify your solution?
In this case, not realistic to match the whole string of the content. So we use file signitures to judge if two files are identical. 
Signitures can include file size, as well as sampled contents on the same positions. They could have different file names and time stamps though.
...
a. Sample to obtain the sliced indices in the strings stored in the RAM only once and used for all the scanned files. Accessing the strings is on-the-fly. 
...
b. Use fast hashing algorithm e.g. MD5 or use SHA256 (no collisions found yet). If no worry about the collision, meaning the hashcode is 1-1. 
Thus in the hashmap, the storage consumption on key string can be replaced by key_hashcode, space usage compressed.

If you can only read the file by 1kb each time, how will you modify your solution?
That is the file cannot fit the whole ram. Use a buffer to read controlled by a loop; read until not needed or to the end. 
