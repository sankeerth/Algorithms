...
is the distance from the top of the rectangular cake to the ith horizontal cut and similarly, verticalCuts[j] is the distance from 
...
class Solution:
    def maxArea(self, h: int, w: int, horizontalCuts: List[int], verticalCuts: List[int]) -> int:
        ...
        for i in range(1, len(horizontalCuts)):
            hCut = horizontalCuts[i] - horizontalCuts[i-1]
            for j in range(1, len(verticalCuts)):
                vCut = verticalCuts[j] - verticalCuts[j-1]
                vCut = vCut % (10**9 + 7)
                res = max(res, hCut * vCut)
