    ...
    def openLock(self, deadends: List[str], target: str) -> int:
        ...
        while queue:
            ...
            for i, c in enumerate(current):
                n = (int(c) + 1) % 10
                lock = current[:i] + str(n) + current[i+1:]
                if lock not in seen:
                    queue.append((lock, count+1))
                    seen.add(lock)
                
                p = (int(c) - 1) % 10
                lock = current[:i] + str(p) + current[i+1:]
                if lock not in seen:
                    queue.append((lock, count+1))
                    seen.add(lock)
...
Initializing the hash maps with n key-value pairs, and the hash set with d combinations of length w will take O(2⋅n) and O(d⋅w) time respectively.
In the worst case, we might iterate on all n^w unique combinations, and for each combination, we perform 2⋅w turns. Thus, it will take O(n^w⋅2⋅w)=O(n^w⋅w) time.
