...
Returns true if the item was present, false otherwise. Note that if val has multiple 
...
(it's guaranteed that at least one element exists when this method is called). The probability of 
each element being returned is linearly related to the number of same values the multiset contains.
...
class RandomizedCollection:
    ...
    def insert(self, val: int) -> bool:
        ...
        """
        isPresent = val in self.val_to_index
        
        if not isPresent:
            self.val_to_index[val] = set()

        self.array.append(val)
        self.val_to_index[val].add(self.count)
        self.count += 1
    ...
    def remove(self, val: int) -> bool:
        ...
        self.val_to_index[lastVal].add(index)

        self.val_to_index[lastVal].remove(self.count-1)
        self.array.pop()
