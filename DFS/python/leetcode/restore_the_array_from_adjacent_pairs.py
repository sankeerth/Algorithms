...
The pairs can appear in any order. Return the original array nums. If there are multiple solutions, return any of them.
...
class Solution:
    def restoreArray(self, adjacentPairs: List[List[int]]) -> List[int]:
        ...
        for num1, num2 in adjacentPairs:
            adjList[num1].append(num2)
            adjList[num2].append(num1)
        ...
        start = min(adjList.items(), key=lambda x: len(x[1]))[0]
        dfs(start)
        return res
