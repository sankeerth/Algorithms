...
class Solution:
    def checkEqualTree(self, root):
        ...
        def postorder(root, parent=None):
            nonlocal result
            if not root:
                return 0
            left = postorder(root.left)
            right = postorder(root.right)
            if parent and total // 2 == left + right + root.val:
                result = True
            else:
...
Cutting an edge means cutting off a proper subtree (i.e., a subtree but not the whole tree). 
I collect the sums of these proper subtrees in a set and check whether half the total tree sum is a possible cut.
...
Alternatively, I collect all subtree sums in a list so that the whole tree's sum is at the end. 
No need for a hash set's searching speed, as I'm searching the collection only once.
